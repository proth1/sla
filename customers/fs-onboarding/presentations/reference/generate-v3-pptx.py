#!/usr/bin/env python3
"""Generate SLA-branded PowerPoint v3: Discovery Deep Dive and Implementation Roadmap.
...
def add_footer(slide, text="SLA Confidential", dark=False):
...
def add_bpmn_desc_slide(title, sp_id, description):
    ...
    add_text_box(slide, Inches(0.6), Inches(1.22), Inches(2.5), Inches(0.36),
                 sp_id, font_size=10, bold=True, color=BRAND_WHITE, alignment=PP_ALIGN.CENTER)
    # Description
    add_text_box(slide, Inches(0.6), Inches(1.9), Inches(12), Inches(4.5),
                 description, font_size=12, color=BRAND_DARK_GRAY)
    add_footer(slide)
    return slide
...
def add_table_slide(title, headers, rows, subtitle=None):
    ...
    if subtitle:
        add_text_box(slide, Inches(0.6), Inches(1.0), Inches(12), Inches(0.4),
                     subtitle, font_size=11, color=BRAND_DARK_GRAY)
    ...
    for j, h in enumerate(headers):
        ...
        hbar.fill.solid()
        hbar.fill.fore_color.rgb = BRAND_BLUE
        hbar.line.fill.background()
        add_text_box(slide, left + Inches(0.1), y_start + Inches(0.05), Inches(col_w - 0.3), Inches(0.3),
                     h, font_size=9, bold=True, color=BRAND_WHITE)

    # Data rows
    for r_idx, row in enumerate(rows):
        row_y = y_start + Inches(0.45 + r_idx * 0.55)
        bg_color = BRAND_WHITE if r_idx % 2 == 0 else BRAND_LIGHT_GRAY
        for j, cell in enumerate(row):
            ...
            add_text_box(slide, left + Inches(0.1), row_y + Inches(0.05),
                         ...
                         cell, font_size=8, color=BRAND_DARK_GRAY)
...
metrics = [("6-9 mo", "Current E2E Cycle", BRAND_ROSE), ("18", "Committees", BRAND_ROSE),
           ("335/yr", "Assessments", BRAND_BLUE), ("24", "Gaps Identified", BRAND_AMBER)]
...
add_text_box(slide, Inches(0.6), Inches(3.1), Inches(6), Inches(0.3),
             "Highest-ROI Investment Areas", font_size=14, bold=True, color=BRAND_DARK_NAVY,
...
for i, (it, ib) in enumerate(investments):
    add_card(slide, Inches(0.6 + i * 4.1), Inches(3.45), Inches(3.9), Inches(1.1), it, ib, BRAND_EMERALD)
...
add_text_box(slide, Inches(0.8), Inches(4.8), Inches(11), Inches(0.5),
             ...
             font_size=10, color=BRAND_DARK_GRAY)
...
for i, (name, issue, color) in enumerate(phases):
    ...
    add_text_box(slide, left, Inches(1.22), Inches(2.3), Inches(0.4),
                 name, font_size=10, bold=True, color=BRAND_WHITE, alignment=PP_ALIGN.CENTER)
...
add_text_box(slide, Inches(0.6), Inches(2.5), Inches(6), Inches(0.3),
             "Critical Bottlenecks", font_size=14, bold=True, color=BRAND_DARK_NAVY,
             font_name='Open Sans Condensed')
bottlenecks = [
...
for i, (bt, bb) in enumerate(bottlenecks):
    ...
    add_card(slide, Inches(0.6 + col * 6.2), Inches(2.9 + row * 1.15),
             Inches(5.9), Inches(1.0), bt, bb, BRAND_ROSE)

# Strengths
add_text_box(slide, Inches(0.6), Inches(5.3), Inches(6), Inches(0.3),
             "What Works Well", font_size=14, bold=True, color=BRAND_DARK_NAVY,
             font_name='Open Sans Condensed')
strengths = [
...
for i, (st, sb) in enumerate(strengths):
    add_card(slide, Inches(0.6 + i * 4.1), Inches(5.6), Inches(3.9), Inches(0.95), st, sb, BRAND_EMERALD)
...
num_metrics = [
    ("335/yr", "Assessments", BRAND_BLUE),
    ("28-29d", "RAE Actual\n(14d target)", BRAND_ROSE),
    ("75d", "DD Internal Review", BRAND_ROSE),
    ("2 ppl", "30+ Contracts/mo", BRAND_ROSE),
]
for i, (val, lab, clr) in enumerate(num_metrics):
    add_metric(slide, Inches(0.6 + i * 3.1), Inches(1.2), val, lab, clr)

num_metrics_2 = [
    ("1.5yr", "Security Exhibit\nNegotiation", BRAND_ROSE),
    ("2-3/10", "Business Council\nQuorum", BRAND_AMBER),
    ("60-90d", "Competitor\nBenchmark", BRAND_LIGHT_BLUE),
    ("18", "Committees\n(target: 5 streams)", BRAND_AMBER),
]
...
add_text_box(slide, Inches(0.6), Inches(1.1), Inches(5.8), Inches(0.3),
             ...
             color=BRAND_DARK_NAVY, font_name='Open Sans Condensed')
arch_today = [
...
for i, (at, ab) in enumerate(arch_today):
    add_card(slide, Inches(0.6), Inches(1.45 + i * 0.95), Inches(5.8), Inches(0.85), at, ab, BRAND_EMERALD)
...
for i, (pt, pb) in enumerate(proposed):
    add_card(slide, Inches(6.8), Inches(1.45 + i * 0.95), Inches(5.8), Inches(0.85), pt, pb, BRAND_LIGHT_BLUE)
...
add_text_box(slide, Inches(0.6), Inches(1.1), Inches(5.8), Inches(0.3),
             ...
             color=BRAND_ROSE, font_name='Open Sans Condensed')
seq_steps = [
    "TBC approval",
...
for i, s in enumerate(seq_steps):
    add_text_box(slide, Inches(0.8), Inches(1.5 + i * 0.32), Inches(5.5), Inches(0.3),
                 f"  {s}", font_size=9, color=BRAND_DARK_GRAY)

# Proposed state - parallel
add_text_box(slide, Inches(6.8), Inches(1.1), Inches(5.8), Inches(0.3),
             "Proposed: Parallel (bounded by slowest SLA)", font_size=14, bold=True,
             color=BRAND_EMERALD, font_name='Open Sans Condensed')
par_streams = [
...
for i, (rt, rd) in enumerate(req_types):
    accent = [BRAND_BLUE, BRAND_AMBER, BRAND_LIGHT_BLUE][i]
...
for i, (pn, pd) in enumerate(pods):
    add_card(slide, Inches(6.8), Inches(1.5 + i * 0.73), Inches(5.8), Inches(0.63),
             pn, f"{pd}  |  Central team ensures consistency", BRAND_MEDIUM_BLUE)
...
roadmap_data = [
    ("Days 1-30: Foundation", BRAND_EMERALD, [
    ...
    ("Days 31-60: Quick Wins", BRAND_LIGHT_BLUE, [
    ...
    ("Days 61-90: Automation", BRAND_MEDIUM_BLUE, [
...
for i, (fs, fd) in enumerate(flow_steps):
    add_card(slide, Inches(0.6 + i * 3.1), Inches(1.5), Inches(2.9), Inches(1.1), fs, fd, BRAND_LIGHT_BLUE)
...
for i, (ip, desc) in enumerate(int_points):
    add_card(slide, Inches(0.6 + i * 4.1), Inches(3.3), Inches(3.9), Inches(1.1), ip, desc, BRAND_BLUE)
...
for i, (vn, vd) in enumerate(variables):
    ...
    add_card(slide, left, top, Inches(5.9), Inches(0.45), vn, vd, BRAND_MEDIUM_BLUE)
...
add_text_box(slide, Inches(1), Inches(0.8), Inches(10), Inches(0.8),
             "Next Steps", font_size=40, bold=True, color=BRAND_WHITE, font_name='Open Sans Condensed')
...
for i, (ns_title, ns_body, accent) in enumerate(next_steps):
    ...
    add_text_box(slide, left + Inches(0.2), top + Inches(0.15), Inches(5.0), Inches(0.4),
                 ns_title, font_size=14, bold=True, color=BRAND_WHITE, font_name='Open Sans Condensed')
    # Body
